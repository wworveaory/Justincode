...
register.addEventListener('click', function () {
  ...
  //3..trim() 是去頭尾空白 防止使用者亂按空白鍵

  //記得true的時候要拿變數進來接收值喔!
  erro += !username.value.trim() ? '你沒填名稱' : ''
  !useraddress.value.trim() ? (erro += '你沒填信箱') : ''
  !useraccount.value.trim() ? (erro += '你沒填帳號') : ''
  !password.value.trim() ? (erro += '你沒填密碼') : ''
  !password2.value.trim() ? (erro += '你沒填確認碼') : ''
